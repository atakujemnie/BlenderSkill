...
def ref_proof(kind: str, pid: str, *, validator: str = "APPEARANCE_REFERENCE_VALIDATE", reg: bool = False) -> dict:
    out = {"status": "PASS", "evidence_kind": kind, "validator_id": validator, "provenance_id": pid, "source_reference_id": "bench_ref_v1"}
    if reg: out["registration_id"] = f"{pid}_reg"
    return out
...
def main() -> None:
    ...
    app_report = {
        "target_fidelity": "L5", "strict_evidence": True,
        "appearance_owner_coverage": {"status": "PASS", "validator_id": "APPEARANCE_OWNER_COVERAGE", "missing_must": []},
        ...
        "detail_coverage": {**ref_proof("DETAIL_COVERAGE", "coverage"), "must_missing": 0, "weighted_coverage": 1.0},
        "reference_fidelity_score": 9.1, "benchmark_score_threshold": 8.5,
    }
    passed = appearance.evaluate(app_report); assert passed["status"] == "PASS", passed
    missing_source = dict(app_report); missing_source["trim_paths"] = dict(app_report["trim_paths"]); missing_source["trim_paths"].pop("source_reference_id")
    assert any(b["reason"] == "missing_source_reference" for b in appearance.evaluate(missing_source)["blockers"])
    missing_reg = dict(app_report); missing_reg["part_boundaries"] = dict(app_report["part_boundaries"]); missing_reg["part_boundaries"].pop("registration_id")
    assert any(b["reason"] == "missing_registration_id" for b in appearance.evaluate(missing_reg)["blockers"])
    low_score = dict(app_report); low_score["reference_fidelity_score"] = 6.0
    assert any(b["owner"] == "benchmark_score" for b in appearance.evaluate(low_score)["blockers"])

    node_report = {
        "node_id": "SIDE_MODULE_R", "strict_evidence": True, "parent_status": "PASS", "required_views": ["SIDE"],
        ...
        "regression": {"status": "PASS", "evidence_kind": "REGRESSION_DIFF", "validator_id": "REFERENCE_OVERLAY_VALIDATE", "provenance_id": "reg"},
        "views": {"SIDE": {"status": "PASS", "evidence_kind": "REGISTERED_OVERLAY", "validator_id": "REFERENCE_OVERLAY_VALIDATE", "provenance_id": "side", "source_reference_id": "side_ref", "registration_id": "side_reg"}},
    }
    assert node_gate.evaluate(node_report)["status"] == "ACCEPTED"
    local_gate = dict(node_report); local_gate["views"] = {"SIDE": dict(node_report["views"]["SIDE"], validator_id="LOCAL_GATE")}
    rejected = node_gate.evaluate(local_gate); assert rejected["status"] == "UNVERIFIED" and any(b["reason"] == "CANONICAL_REFERENCE_VIEW_VALIDATOR_REQUIRED" for b in rejected["blockers"])
    ...
    fidelity_report = {
        ...
        "canonical_views": {view: fidelity_ref("REGISTERED_OVERLAY", view.lower(), "REFERENCE_OVERLAY_VALIDATE", reg=True) for view in ("FRONT", "SIDE", "TOP", "REAR", "BOTTOM")},
        "must_features": [fidelity_ref("FEATURE_ROI", "trim_feature", "APPEARANCE_REFERENCE_VALIDATE", reg=True) | {"id": "SIDE_TRIM_R"}],
    }
