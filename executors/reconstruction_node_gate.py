...
v0.12 adds mandatory mutation-postcondition and assembly-integrity evidence for
authorized production mutations while retaining v0.11 per-view/provenance rules.
"""
...
CANONICAL_OWNER_VALIDATORS = {
    "QA_SCENE_ISOLATE", "REFERENCE_MEASURE", "REFERENCE_OVERLAY_VALIDATE",
    "APPEARANCE_REFERENCE_VALIDATE", "SECTION_LOFT_HARD_SURFACE",
    "LAYER_STACK_VALIDATE", "MESH_VALIDATE", "REFERENCE_CONFLICT_RESOLVER",
    "RECONSTRUCTION_NODE_GATE", "MUTATION_POSTCONDITION_GATE",
    "ASSEMBLY_INTEGRITY_GATE", "VALIDATOR_NEGATIVE_CONTROL",
}


def _record_status(value: Any, *, strict: bool = True,
                   require_view_validator: bool = False,
                   allowed_evidence: set[str] | None = None) -> tuple[str, str | None]:
    ...
    if allowed_evidence is not None and evidence not in allowed_evidence:
        return "UNVERIFIED", f"VIEW_EVIDENCE_KIND_NOT_ALLOWED:{evidence}"
    ...
    if evidence in REFERENCE_DERIVED_EVIDENCE and not (
...
def evaluate(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if auth is not None:
        if not isinstance(auth, Mapping) or str(auth.get("validator_id", "")).upper() != "EXECUTION_AUTHORIZATION_GATE":
            blockers.append({"owner": "authorization", "status": "UNVERIFIED", "reason": "CANONICAL_EXECUTION_AUTHORIZATION_REQUIRED"})
        elif str(auth.get("status", "")).upper() != PASS:
            blockers.append({"owner": "authorization", "status": "FAIL", "reason": "EXECUTION_AUTHORIZATION_NOT_PASS"})
    ...
    # v0.12 hardening: once a production mutation was authorized, the node cannot
    # be accepted until the mutation is proven to have actually changed geometry
    # as intended and its assembly relations are geometrically valid.
    if auth is not None:
        required_single["mutation_postcondition"] = report.get("mutation_postcondition")
        required_single["assembly_integrity"] = report.get("assembly_integrity")
    ...
    if auth is not None:
        mut = report.get("mutation_postcondition")
        if isinstance(mut, Mapping) and str(mut.get("status", "")).upper() == NOT_REQUIRED:
            blockers.append({"owner": "mutation_postcondition", "status": "UNVERIFIED", "reason": "MUTATION_POSTCONDITION_CANNOT_BE_WAIVED"})
        assembly = report.get("assembly_integrity")
        if isinstance(assembly, Mapping) and str(assembly.get("status", "")).upper() == NOT_REQUIRED:
            if str(assembly.get("validator_id", "")).upper() != "ASSEMBLY_INTEGRITY_GATE" or int(assembly.get("relation_count", -1)) != 0 or not assembly.get("provenance_id"):
                blockers.append({"owner": "assembly_integrity", "status": "UNVERIFIED", "reason": "ASSEMBLY_NOT_REQUIRED_REQUIRES_CANONICAL_ZERO_RELATION_PROOF"})
    ...
    for view in required_views:
        status, reason = _record_status(views.get(view), strict=strict,
                                        ...
                                        allowed_evidence=_view_contract(report, view))
        if status != PASS:
            blockers.append({"owner": f"view:{view}", "status": status,
                             "reason": reason or "REQUIRED_VIEW_NOT_PASS"})

    for item in list(report.get("derived_parameters", [])):
        if not isinstance(item, Mapping):
            blockers.append({"owner": "derived_parameter", "status": "UNVERIFIED", "reason": "INVALID_DERIVED_PARAMETER_RECORD"})
            continue
        param_id = str(item.get("id", "UNKNOWN"))
        required = ["value", "method", "source_reference_id", "confidence", "provenance_id"]
        missing = [key for key in required if item.get(key) is None]
        if missing:
            blockers.append({"owner": f"derived_parameter:{param_id}", "status": "UNVERIFIED",
                             "reason": "DERIVED_PARAMETER_PROVENANCE_MISSING:" + ",".join(missing)})
        if item.get("conflict_decision_required") and not item.get("conflict_decision_id"):
            blockers.append({"owner": f"derived_parameter:{param_id}", "status": "UNVERIFIED", "reason": "CONFLICT_DECISION_REQUIRED"})
    ...
    for dev in list(report.get("deviations", [])):
        ...
        if severity in {"HARD", "MUST", "CANONICAL"} and status not in {"RESOLVED", "ACCEPTED_BY_AUTHORITY"}:
            blockers.append({"owner": owner, "status": status, "reason": "UNRESOLVED_HARD_DEVIATION"})
        if status == "ACCEPTED_BY_AUTHORITY" and (not dev.get("authority_source") or not dev.get("authority_record_id")):
            blockers.append({"owner": owner, "status": "UNVERIFIED", "reason": "AUTHORITY_PROVENANCE_REQUIRED"})

    status = "ACCEPTED" if not blockers else "FAIL"
    if blockers and all(b["status"] in {"UNVERIFIED", "NOT_EVALUATED"} for b in blockers):
        status = "UNVERIFIED"
    if any(b["reason"] in {"PARENT_NOT_ACCEPTED", "DEPENDENCY_NOT_ACCEPTED", "EXECUTION_AUTHORIZATION_NOT_PASS"} for b in blockers):
        status = "BLOCKED"
    return {"status": status, "node_id": node_id,
            "graph_revision": report.get("graph_revision"),
            "node_revision": report.get("node_revision"),
            "accepted": status == "ACCEPTED", "children_unlocked": status == "ACCEPTED",
            "blockers": blockers, "strict_reference_anchoring": strict,
            "canonical_view_validators": sorted(CANONICAL_VIEW_VALIDATORS),
            "geometric_integrity_required": auth is not None}
