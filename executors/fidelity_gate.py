...
The caller supplies compact validator reports; this executor aggregates
ownership, evidence class and authority closure and decides whether R12+ may
begin.

A bare ``status: PASS`` is intentionally insufficient in strict v0.8 mode.
"""
    ...
    *,
    ...
    if st != "PASS":
        return {
            "owner": owner,
            "status": st,
            "reason": "required_gate_not_passed",
        }
    ...
    if not isinstance(value, Mapping):
        return {
            "owner": owner,
            "status": "UNVERIFIED",
            "reason": "pass_without_evidence_record",
        }
...
def evaluate(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    required_single = [
        (
            "hard_dimensions",
            report.get("hard_dimensions"),
            "hard_dimensions",
        ),
        (
            "landmarks_d0_d1",
            report.get("landmarks_d0_d1"),
            "landmarks_d0_d1",
        ),
    ]
    if LEVELS[target] >= 4:
        required_single.append(
            (
                "material_segmentation",
                report.get("material_segmentation"),
                "material_segmentation",
            )
        )
    ...
    required_views = list(
        report.get("required_views", ["FRONT", "SIDE", "TOP", "REAR", "BOTTOM"])
    )
    ...
    return {
        ...
        "next_state": (
            "R12_TOPOLOGY_RUNTIME"
            if not blockers
            else "BACKTRACK_TO_EARLIEST_FIDELITY_OWNER"
        ),
    }
