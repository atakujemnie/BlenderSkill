...
v0.12 hardening:
- target L4/L5 requires GEOMETRIC_INTEGRITY_GATE in addition to appearance;
- physical integrity is non-compensating and cannot be averaged away by source fit.
"""
...
ALLOWED_EVIDENCE = {
    ...
    "must_feature": {
        "FEATURE_ROI", "LAYER_STACK", "RAY_VISIBILITY", "NUMERIC_MEASUREMENT",
        "REGISTERED_OVERLAY", "SILHOUETTE_DIFF", "LANDMARK_PROJECTION",
        "PART_BOUNDARY_VALIDATION", "TRIM_PATH_VALIDATION", "JUNCTION_VALIDATION",
        "EDGE_FAMILY_VALIDATION", "MATERIAL_APPEARANCE_VALIDATION",
        "EMISSIVE_REGION_VALIDATION", "DETAIL_COVERAGE",
    },
    "appearance_fidelity": {"APPEARANCE_FIDELITY_GATE"},
    "geometric_integrity": {"GEOMETRIC_INTEGRITY_GATE"},
}
...
ALLOWED_VALIDATORS = {
    ...
    "must_feature": {"REFERENCE_OVERLAY_VALIDATE", "APPEARANCE_REFERENCE_VALIDATE", "LAYER_STACK_VALIDATE", "REFERENCE_MEASURE"},
    "appearance_fidelity": {"APPEARANCE_FIDELITY_GATE"},
    "geometric_integrity": {"GEOMETRIC_INTEGRITY_GATE"},
}

REFERENCE_PROOF_CLASSES = {"hard_dimensions", "landmarks_d0_d1", "material_segmentation", "canonical_view", "must_feature"}
PROJECTED_EVIDENCE = {"REGISTERED_OVERLAY", "SILHOUETTE_DIFF", "FEATURE_ROI", "LANDMARK_PROJECTION", "PART_BOUNDARY_VALIDATION", "TRIM_PATH_VALIDATION", "JUNCTION_VALIDATION"}


def _status(item: Any) -> str:
    if isinstance(item, str): return item.upper()
    if isinstance(item, Mapping): return str(item.get("status", "UNVERIFIED")).upper()
    return "UNVERIFIED"


def _proof_blocker(owner: str, value: Any, *, proof_class: str,
                   strict_evidence: bool) -> dict[str, str] | None:
    st = _status(value)
    if st != "PASS":
        return {"owner": owner, "status": st, "reason": "required_gate_not_passed"}
    if not strict_evidence: return None
    ...
    if evidence_kind not in ALLOWED_EVIDENCE[proof_class]:
        return {"owner": owner, "status": "UNVERIFIED", "reason": f"invalid_or_missing_evidence_kind:{evidence_kind or 'NONE'}"}
    provenance_id = str(value.get("provenance_id") or value.get("registration_id") or value.get("artifact_id") or "").strip()
    ...
    if validator not in ALLOWED_VALIDATORS[proof_class]:
        return {"owner": owner, "status": "UNVERIFIED", "reason": f"invalid_or_missing_validator:{validator or 'NONE'}"}
...
def evaluate(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if LEVELS[target] >= 4:
        required_single.extend([
            ...
            ("geometric_integrity", report.get("geometric_integrity"), "geometric_integrity"),
        ])

    for owner, value, proof_class in required_single:
        blocker = _proof_blocker(owner, value, proof_class=proof_class, strict_evidence=strict_evidence)
    ...
    for view in required_views:
        blocker = _proof_blocker(f"view:{view}", canonical.get(view), proof_class="canonical_view", strict_evidence=strict_evidence)
        if blocker:
            if blocker["reason"] == "required_gate_not_passed": blocker["reason"] = "canonical_view_not_passed"
            blockers.append(blocker)

    for feature in list(report.get("must_features", [])):
        fid = str(feature.get("id", "UNKNOWN"))
        blocker = _proof_blocker(f"feature:{fid}", feature, proof_class="must_feature", strict_evidence=strict_evidence)
        if blocker:
            if blocker["reason"] == "required_gate_not_passed": blocker["reason"] = "MUST_feature_not_passed"
            blockers.append(blocker)

    for dev in list(report.get("deviations", [])):
        severity = str(dev.get("severity", "SOFT")).upper()
        st = str(dev.get("status", "OPEN")).upper()
        if severity not in {"HARD", "MUST", "CANONICAL"}: continue
        owner = f"deviation:{dev.get('id', 'UNKNOWN')}"
        if st not in {"RESOLVED", "ACCEPTED_BY_AUTHORITY"}:
            blockers.append({"owner": owner, "status": st, "reason": "unresolved_hard_deviation"}); continue
        if strict_evidence and st == "ACCEPTED_BY_AUTHORITY":
            if not str(dev.get("authority_source", "")).strip() or not str(dev.get("authority_record_id", "")).strip():
                blockers.append({"owner": owner, "status": "UNVERIFIED", "reason": "authority_acceptance_without_record"})
        if strict_evidence and st == "RESOLVED":
            resolution_record = str(dev.get("resolution_record_id") or dev.get("provenance_id") or "").strip()
            if not resolution_record:
                blockers.append({"owner": owner, "status": "UNVERIFIED", "reason": "resolved_deviation_without_evidence"})
    ...
    return {"status": "PASS" if not blockers else "FAIL", "strict_evidence": strict_evidence,
            "target_fidelity": target, "achieved_fidelity": achieved, "blockers": blockers,
