...
"""Aggregate proof-bearing appearance-fidelity records.

v0.10 release driver: Lafar Street Bench v0.9 showed that correct bounds,
outer silhouette and runtime packaging can coexist with a visibly weak
reconstruction. This gate treats internal product architecture, edge language,
material appearance and detail coverage as non-compensating acceptance owners.
"""
...
ALLOWED_EVIDENCE = {
    "part_boundaries": {"PART_BOUNDARY_VALIDATION", "REGISTERED_OVERLAY", "FEATURE_ROI"},
    "trim_paths": {"TRIM_PATH_VALIDATION", "REGISTERED_OVERLAY", "FEATURE_ROI", "LANDMARK_PROJECTION"},
    "junctions": {"JUNCTION_VALIDATION", "FEATURE_ROI", "LAYER_STACK", "REGISTERED_OVERLAY"},
    "edge_families": {"EDGE_FAMILY_VALIDATION", "FEATURE_ROI", "REGISTERED_OVERLAY", "NUMERIC_MEASUREMENT"},
    "material_regions": {"MATERIAL_APPEARANCE_VALIDATION", "MATERIAL_SEGMENTATION", "REGISTERED_OVERLAY"},
    "emissive_regions": {"EMISSIVE_REGION_VALIDATION", "FEATURE_ROI", "MATERIAL_APPEARANCE_VALIDATION"},
    "branding": {"BRANDING_VALIDATION", "FEATURE_ROI", "REGISTERED_OVERLAY"},
...
def _proof_blocker(owner: str, value: Any, proof_class: str, *, strict: bool) -> dict[str, str] | None:
    ...
    if kind in REFERENCE_DERIVED_KINDS:
...
def _coverage_blocker(value: Any, *, strict: bool, require_complete: bool) -> dict[str, str] | None:
    ...
    if require_complete and missing != 0:
        return {"owner": "detail_coverage", "status": "FAIL", "reason": f"missing_must_features:{missing}"}
    if require_complete and coverage < 0.999999:
        return {"owner": "detail_coverage", "status": "FAIL", "reason": f"must_coverage_incomplete:{coverage:.6f}"}
    return None
...
def evaluate(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if benchmark_threshold is not None and score is not None:
        if float(score) < float(benchmark_threshold):
            blockers.append({
