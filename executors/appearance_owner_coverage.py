...
def evaluate(contract: Mapping[str, Any], report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    expected = {
        ...
        for owner in owners
        if str(owner.get("importance", "MUST")).upper() == "MUST"
    }
    ...
    for owner_id in sorted(expected):
        ...
        elif status not in PASS_LIKE:
            unverified.append(owner_id)
    missing = sorted(expected - accounted)
    if missing:
        blockers.append({"reason": "MUST_OWNER_MISSING", "ids": missing})
    if failed:
        blockers.append({"reason": "MUST_OWNER_FAIL", "ids": failed})
    if unverified:
        blockers.append({"reason": "MUST_OWNER_UNVERIFIED", "ids": unverified})
    return {
        "status": "PASS" if not blockers else "FAIL",
        "validator_id": EXECUTOR_ID,
        "contract_revision": contract.get("revision"),
        "expected_must": len(expected),
        "accounted_must": len(accounted),
        "missing_must": missing,
        "failed_must": failed,
        "unverified_must": unverified,
