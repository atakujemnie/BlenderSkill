...
"""Pure-Python state-transition and checkpoint validator for v0.11 reconstruction."""
...
def _proof_ok(record: Any, validator_id: str) -> bool:
    return isinstance(record, Mapping) and str(record.get("status", "")).upper() in {"PASS", "ACCEPTED"} and str(record.get("validator_id", "")).upper() == validator_id and bool(record.get("provenance_id") or record.get("authorization_id") or record.get("artifact_id"))
...
def validate_transition(old_state: str, new_state: str, *, evidence: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not blockers and new_state == "BUILT_UNVERIFIED" and not _proof_ok(evidence, "LOCAL_BUILDER"):
    ...
    if not blockers and new_state == "DIRTY" and (not isinstance(evidence, Mapping) or not (evidence.get("change_id") or evidence.get("dirty_reason"))):
        blockers.append({"reason": "DIRTY_REASON_REQUIRED"})
    return {"status": "PASS" if not blockers else "FAIL", "validator_id": EXECUTOR_ID, "old_state": old_state, "new_state": new_state, "blockers": blockers, "can_transition": not blockers}


def validate_checkpoint(checkpoint: Mapping[str, Any]) -> dict[str, Any]:
    blockers: list[dict[str, Any]] = []
    if not checkpoint.get("asset_id"):
        blockers.append({"reason": "ASSET_ID_REQUIRED"})
    if not checkpoint.get("graph_revision"):
        blockers.append({"reason": "GRAPH_REVISION_REQUIRED"})
    if not checkpoint.get("state_revision"):
        blockers.append({"reason": "STATE_REVISION_REQUIRED"})
    ...
    if not isinstance(shape_nodes, Mapping):
        blockers.append({"reason": "SHAPE_NODE_NAMESPACE_REQUIRED"})
    if not isinstance(appearance_owners, Mapping):
        blockers.append({"reason": "APPEARANCE_OWNER_NAMESPACE_REQUIRED"})
    ...
    return {"status": "PASS" if not blockers else "FAIL", "validator_id": EXECUTOR_ID, "asset_id": checkpoint.get("asset_id"), "graph_revision": checkpoint.get("graph_revision"), "state_revision": checkpoint.get("state_revision"), "blockers": blockers}


def apply_transition(checkpoint: Mapping[str, Any], node_id: str, new_state: str, *, evidence: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if evidence:
        node["last_transition_provenance"] = evidence.get("provenance_id") or evidence.get("authorization_id") or evidence.get("artifact_id")
    shape_nodes[node_id] = node
    history = list(cp.get("history", []))
    history.append({"node_id": node_id, "from": old_state, "to": new_state.upper(), "provenance": node.get("last_transition_provenance")})
    cp["history"] = history
